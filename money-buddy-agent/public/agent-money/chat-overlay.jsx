...
function ChatOverlay({
  open, onClose, accent, accentInk, avatarVariant,
  liveData, onUpdateData, currentScreen, onNavigate,
}) {
...
今天想聊哪个？`;
  ...
  async function send(text) {
    ...
    if (result.updates) onUpdateData?.(result.updates);
    if (result.navigate) {
      setTimeout(() => onNavigate?.(result.navigate), 600);
    }
  }
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ flex: 1, marginTop: 4 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              ...
              <span style={{ fontSize: 10.5, color: '#9a9a96', fontWeight: 600 }}>
                实时同步 · 不推销产品
              </span>
            </div>
          </div>
        ...
        <div className="app-scroll" style={{
          ...
          {messages.map(m => (
            <ChatRow key={m.id} m={m} accent={accent} accentInk={accentInk}
              avatarVariant={avatarVariant} onNavigate={onNavigate}/>
          ))}
        ...
        <div style={{
          ...
          <input
            ...
            placeholder={typing ? '搭子在想…' : '问我任何理财问题…'}
            disabled={typing}
            style={{
          ...
          />
          <button onClick={() => send()} disabled={!input.trim() || typing} className="press" style={{
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, marginBottom: 10 }}>
      ...
      <div style={{
        ...
        {m.action && (
          <button onClick={() => onNavigate?.(m.action.to)} className="press" style={{
            ...
            {m.action.label} <Icon.Chevron size={12} color="#fff"/>
          </button>
        )}
      </div>
    </div>
...
8) 涉及具体功能页面时可建议 action 跳转`;
  ...
  if (!json || !json.reply) return mockReply(userText, liveData);
  return json;
}
