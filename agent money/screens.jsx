    ...
    <ScreenShell label="01 Home">
      ...
      <div className="dotmask" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, margin: '12px 0 14px' }}>
          <PulseNum value={data?.dailyAvg ?? 72} pulseKey={pulse?.dailyAvg} style={{ fontSize: 72, fontWeight: 900, lineHeight: 0.9 }}/>
        ...
        <div style={{
          ...
          {[
            { l: '已存', k: 'monthlySaved', v: data?.monthlySaved ?? 408, u: `/ ${data?.monthlyGoal ?? 600} 元` },
            { l: '日预算', k: 'dailyBudget', v: data?.dailyBudget ?? 84, u: '元/天' },
            { l: '连击', k: null, v: 5, u: '天内未超支' },
          ].map((o) => (
            <div key={o.l}>
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)', fontWeight: 600, marginBottom: 2 }}>{o.l.toUpperCase()}</div>
              <div style={{ fontSize: 18, fontWeight: 800 }}>
                <PulseNum value={o.v} pulseKey={o.k ? pulse?.[o.k] : 0}/> <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.55)', fontWeight: 500 }}>{o.u}</span>
              </div>
            </div>
          ))}
        </div>
    ...
    <ScreenShell label="02 Plan">
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, marginTop: 14 }}>
          <PulseNum value={data?.dailyBudget ?? 84} pulseKey={pulse?.dailyBudget} style={{ fontSize: 92, fontWeight: 900, lineHeight: 0.85 }}/>
        ...
        <div style={{
          ...
          {[
            { l: '每周',     k: 'weeklyBudget',     v: data?.weeklyBudget ?? 588,        u: '元' },
            { l: '本月可花', k: 'monthlySpendable', v: (data?.monthlySpendable ?? 2520).toLocaleString(), u: '元' },
            { l: '目标',     k: 'monthlyGoal',      v: data?.monthlyGoal ?? 600,         u: '元' },
          ].map(o => (
            <div key={o.l}>
              <div style={{ fontSize: 10, fontWeight: 700, opacity: 0.55 }}>{o.l.toUpperCase()}</div>
              <div style={{ fontSize: 19, fontWeight: 800, marginTop: 2 }}>
                <PulseNum value={o.v} pulseKey={pulse?.[o.k]}/>
                <span style={{ fontSize: 10, opacity: 0.55, marginLeft: 2 }}>{o.u}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <span style={{
            ...
            <Icon.ArrowUp size={10} color={accentInk}/> {(data?.todayTotal ?? total) > (data?.dailyBudget ?? 84) ? `超支 ¥${(data?.todayTotal ?? total) - (data?.dailyBudget ?? 84)}` : '在预算内'}
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, marginTop: 12 }}>
          <PulseNum value={data?.todayTotal ?? total} pulseKey={pulse?.todayTotal} style={{ fontSize: 80, fontWeight: 900, lineHeight: 0.85 }}/>
        ...
        <div style={{ marginTop: 6, fontSize: 12, color: 'rgba(255,255,255,0.55)' }}>
          预算 ¥{data?.dailyBudget ?? 84} · 用掉 <b style={{ color: '#fff', fontWeight: 800 }}>{Math.round((data?.todayTotal ?? total) / (data?.dailyBudget ?? 84) * 100)}%</b>
        </div>
